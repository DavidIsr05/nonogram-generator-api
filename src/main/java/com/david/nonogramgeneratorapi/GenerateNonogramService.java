...
public class GenerateNonogramService {
  ...
  static {
    try {
      System.load(
          "/Users/david/Documents/nonogramGeneratorAPI/src/main/resources/libopencv_java4120.dylib"); // TODO change the dir so it will work in container also
  ...
  public void initModel() throws Exception {
    try {
      ...
      if (this.net.empty()) {
        throw new CouldNotLoadModelException(
            "Could not load model. Model variable empty after trying to load it.");
      }
  ...
  public nonogramResponseDto generateNonogram(nonogramGenerationRequestDto requestBody)
      throws Exception {
    ...
    BufferedImage modifiedBufferedImage =
        applyMaskFromModel(
            originalImageFile.getAbsoluteFile(),
            maskFromModelBufferedImage,
            requestBody.getPixelHighlightValue());

    int matrixSize = requestBody.getDifficulty().getMatrixSize();

    BufferedImage scaledBufferedImage =
        Scalr.resize(
            modifiedBufferedImage,
            Scalr.Method.ULTRA_QUALITY,
            Scalr.Mode.FIT_EXACT,
            matrixSize,
            Scalr.OP_ANTIALIAS);

    BufferedImage originalScaled =
        Scalr.resize(
            originalBufferImage,
            Scalr.Method.ULTRA_QUALITY,
            Scalr.Mode.FIT_EXACT,
    ...
    BufferedImage grayScaleBufferImage =
        new BufferedImage(matrixSize, matrixSize, BufferedImage.TYPE_BYTE_GRAY);
    ...
    BufferedImage blackAndWhiteBufferedImage =
        generateBlackAndWhiteImage(grayScaleBufferImage, threshold);
    ...
    ImageIO.write(
        highlightOriginalImageBasedOnBlackAndWhiteImage(
    ...
    String originalDownscaledImageBase64 =
        Base64.getEncoder().encodeToString(originalDownscaledFileContent);

    if (!previewFile.delete())
      throw new CouldNotDeleteFileException(
          "Could not delete " + previewFile + " file with path: " + previewFile.getAbsolutePath());
    if (!originalDownscaledFile.delete())
      throw new CouldNotDeleteFileException(
          "Could not delete "
              + originalDownscaledFile
              + " file with path: "
              + originalDownscaledFile.getAbsolutePath());
  ...
  private void detectMainObjectUsingModel(String inputPath, String outputPath) throws Exception {
    ...
    if (imageFromInputPath.empty())
      throw new FileNotFoundException(
          "Problem while loading original image for model in: 'detectMainObjectUsingModel', with path: "
              + inputPath);

    Mat mainObjectFromModel =
        Dnn.blobFromImage(
            imageFromInputPath, 0.01, new Size(250, 250), new Scalar(0, 0, 0), true, false);
  ...
  private BufferedImage applyMaskFromModel(
      File originalImageFile, BufferedImage maskFromModelBufferedImage, double pixelHighlightValue)
      throws IOException {
    ...
    for (int imageYIndex = 0; imageYIndex < originalBufferedImage.getHeight(); imageYIndex++) {
      for (int imageXIndex = 0; imageXIndex < originalBufferedImage.getWidth(); imageXIndex++) {
        boolean isNotMainObjectPixel =
            calculatePixelBrightness(maskFromModelBufferedImage, imageYIndex, imageXIndex) == 0;
        int originalPixel = originalBufferedImage.getRGB(imageXIndex, imageYIndex);

        int updatedPixel =
            getUpdatedPixel(originalPixel, isNotMainObjectPixel, pixelHighlightValue);
  ...
  private static int getUpdatedPixel(
      int originalPixel, boolean isNotMainObjectPixel, double pixelHighlightValue) {
    if (!isNotMainObjectPixel) {
      ...
      final int newGreen =
          Math.min(255, Math.max(0, (int) (color.getGreen() * pixelHighlightValue)));
  ...
  private int calculateAverageBrightnessOfImage(
      BufferedImage grayScaleBufferImage, int matrixSize) {
  ...
  private BufferedImage generateBlackAndWhiteImage(
      BufferedImage grayScaleBufferImage, int threshold) {
  ...
  private boolean[][] generateNonogram(BufferedImage blackAndWhiteImage) {
    boolean[][] nonogram =
        new boolean[blackAndWhiteImage.getWidth()][blackAndWhiteImage.getHeight()];
  ...
  private BufferedImage highlightOriginalImageBasedOnBlackAndWhiteImage(
      BufferedImage blackAndWhiteImage,
    ...
    BufferedImage previewOverlayBufferImage =
        new BufferedImage(
    ...
    for (int blackAndWhiteImageXIndex = 0;
        blackAndWhiteImageXIndex < blackAndWhiteImage.getWidth();
        blackAndWhiteImageXIndex++) {
      for (int blackAndWhiteImageYIndex = 0;
          blackAndWhiteImageYIndex < blackAndWhiteImage.getHeight();
          blackAndWhiteImageYIndex++) {

        boolean isPixelBlack =
            calculatePixelBrightness(
                    blackAndWhiteImage, blackAndWhiteImageYIndex, blackAndWhiteImageXIndex)
                < 128;

        if (isPixelBlack) {

          int coordinateXOnOriginalBasedOnBlackAndWhiteImage =
              blackAndWhiteImageXIndex * pixelWidthRatio;
          int coordinateYOnOriginalBasedOnBlackAndWhiteImage =
              blackAndWhiteImageYIndex * pixelHeightRatio;
          ...
          g2d.setComposite(
              AlphaComposite.getInstance(
                  AlphaComposite.SRC_OVER, pixelHighlightValueBasedOnImageAverageBrightness));
          g2d.drawOval(
              coordinateXOnOriginalBasedOnBlackAndWhiteImage,
              coordinateYOnOriginalBasedOnBlackAndWhiteImage,
              pixelWidthRatio,
              pixelHeightRatio);
        }
      }
