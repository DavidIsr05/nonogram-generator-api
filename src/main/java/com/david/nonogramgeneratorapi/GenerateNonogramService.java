...
public class GenerateNonogramService {
    ...
    public nonogramResponseDto generateNonogram(nonogramGenerationRequestDto requestBody) throws Exception {
        ...
        BufferedImage scaledBufferedImage = Scalr.resize(modifiedBufferedImage, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.FIT_EXACT, matrixSize, Scalr.OP_ANTIALIAS);

        BufferedImage originalScaled = Scalr.resize(originalBufferImage, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.FIT_EXACT, matrixSize, Scalr.OP_ANTIALIAS);
        ...
        if (originalBufferImage.getHeight() > 500 | originalBufferImage.getWidth() > 500) {
            originalImageDownscaledForPreview = Scalr.resize(originalBufferImage, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.AUTOMATIC, 500, Scalr.OP_ANTIALIAS);
        }
        File previewFile = new File(outputPath + "/previw.png");
        ImageIO.write(highlightOriginalImageBasedOnBlackAndWhiteImage(blackAndWhiteBufferedImage, originalImageDownscaledForPreview, threshold, requestBody.getPreviewImageHighlightColor().getColor()), "png", previewFile);

        if (!originalImageFile.delete())
            throw new CouldNotDeleteFileException("Could not delete " + originalImageFile + " file with path: " + originalImageFile.getAbsolutePath());
        if (!processedFile.delete())
            throw new CouldNotDeleteFileException("Could not delete " + processedFile + " file with path: " + processedFile.getAbsolutePath());
        ...
        if (!previewFile.delete())
            throw new CouldNotDeleteFileException("Could not delete " + previewFile + " file with path: " + previewFile.getAbsolutePath());
        if (!originalDownscaledFile.delete())
            throw new CouldNotDeleteFileException("Could not delete " + originalDownscaledFile + " file with path: " + originalDownscaledFile.getAbsolutePath());
    ...
    private void detectMainObjectUsingModel(String inputPath, String outputPath) throws Exception {
        ...
        if (imageFromInputPath.empty())
            throw new FileNotFoundException("Problem while loading original image for model in: 'detectMainObjectUsingModel', with path: " + inputPath);
    ...
    private BufferedImage generateBlackAndWhiteImage(BufferedImage grayScaleBufferImage, int threshold) {
        BufferedImage blackAndWhiteImage = new BufferedImage(grayScaleBufferImage.getWidth(), grayScaleBufferImage.getHeight(), BufferedImage.TYPE_BYTE_BINARY);
    ...
    private BufferedImage highlightOriginalImageBasedOnBlackAndWhiteImage(BufferedImage blackAndWhiteImage, BufferedImage originalImage, int threshold, Color highlightColor) {
        ...
        BufferedImage previewOverlayBufferImage = new BufferedImage(originalImage.getWidth(), originalImage.getHeight(), BufferedImage.TYPE_INT_ARGB);
